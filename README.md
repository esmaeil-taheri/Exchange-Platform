...
| `purge_expired_idempotency_records` | Periodic (Beat) | Drop idempotency keys past their replay window |
| `report_stuck_idempotency_records` | Periodic (Beat) | Alert on keys claimed but never resolved |

### Beat entries to register

Beat schedules live in the database (`django_celery_beat`), so new periodic
tasks must be added from Django Admin — they are not picked up from code:

| Task | Suggested interval |
|------|--------------------|
| `purge_expired_idempotency_records` | hourly |
| `report_stuck_idempotency_records` | every 15 minutes |
...
Every endpoint that moves money accepts an `Idempotency-Key` request header:

| Endpoint | Method |
|----------|--------|
| `/api/v1/exchange/buy/` | POST |
| `/api/v1/exchange/sell/` | POST |
| `/api/v1/payments/deposit/` | POST |
| `/api/v1/settlements/withdrawal/` | POST |
...
* The key is generated by the **client**, once per user *intent* — a UUID4 at
  the moment the customer taps the button — and resent unchanged on every
  retry, including after a timeout. A key generated per *attempt* protects
  nothing.
* A new operation must carry a new key.
* Format: 8–64 characters of `A-Z a-z 0-9 _ - : .`
...
| Repeat, same key and body | The original response, replayed verbatim |
| Repeat, same key, different body | `422` — the key was reused for a different request |
| Repeat while the first is still running | `409` + `Retry-After` — retry the same key |
| First request failed | Key is released; the retry is a real retry |
| Malformed or (when required) missing key | `400` |

**Why a failed request frees its key:** for the wallet-funded paths the claim is
written inside the same database transaction as the debit, so a rollback takes
both. That is also why a duplicate can never land between the two.

**Rollout:** `IDEMPOTENCY_REQUIRED` defaults to `False`, so the header is
honoured when present and endpoints behave exactly as before when it is absent.
The protection is only real for clients that send a key — set
`IDEMPOTENCY_REQUIRED=True` once every client has been updated.

| Setting | Default | Purpose |
|---------|---------|---------|
| `IDEMPOTENCY_REQUIRED` | `False` | Reject requests with no key |
| `IDEMPOTENCY_RETENTION_HOURS` | `48` | How long a response stays replayable |
| `IDEMPOTENCY_IN_PROGRESS_TIMEOUT_SECONDS` | `120` | When a crashed gateway claim becomes reclaimable |
